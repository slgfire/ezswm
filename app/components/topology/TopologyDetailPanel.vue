<template>
  <div
    v-if="node"
    class="flex h-full w-[290px] flex-col border-l border-default bg-default overflow-y-auto"
  >
    <div class="p-5">
      <!-- Header -->
      <div class="flex items-start justify-between gap-2 mb-1">
        ...
        <UBadge v-if="node.role" :color="roleColor(node.role)" variant="subtle" size="sm" class="shrink-0">
          {{ node.role }}
        </UBadge>
      </div>
      <p v-if="node.manufacturer || node.model" class="text-sm text-gray-500 dark:text-gray-400 mb-4">
      ...
      <!-- Port footer -->
      <div class="flex items-center justify-between border-t border-b border-default py-2 mb-4 font-mono">
      ...
      <!-- Connections -->
      <div v-if="groupedConnections.length" class="mb-4">
      ...
      <NuxtLink
        ...
        class="block w-full rounded-lg border border-green-500/15 bg-green-500/8 px-4 py-2.5 text-center text-sm font-medium text-green-500 transition-colors hover:bg-green-500/15"
      >
