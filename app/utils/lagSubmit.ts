...
export function buildLagSaveRequest(options: { switchId: string; lagId?: string; siteId?: string; isEdit: boolean; isDuplicate: boolean; body: Record<string, unknown> }) {
  return { url: `/api/switches/${options.switchId}/lag-groups${options.isEdit && options.lagId ? `/${options.lagId}` : ''}`, method: options.isEdit && !options.isDuplicate ? 'PUT' as const : 'POST' as const, body: options.body, query: options.siteId ? { siteId: options.siteId } : undefined }
}
