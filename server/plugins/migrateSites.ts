...
export default defineNitroPlugin(() => {
  ...
  const sites = readJson<any[]>('sites.json')
  ...
  if (sites.length === 0) {
    ...
    // Switches
    const switches = readJson<any[]>('switches.json')
    ...
    // VLANs
    const vlans = readJson<any[]>('vlans.json')
    if (vlans.length > 0) {
    ...
    // Networks
    const networks = readJson<any[]>('networks.json')
