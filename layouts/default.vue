...
<template>
  <div class="min-h-screen bg-[var(--ui-bg)] text-[var(--ui-text)]">
    <div class="relative min-h-screen lg:pl-72">
      <aside class="fixed inset-y-0 left-0 z-40 hidden w-72 border-r border-default/70 bg-[var(--ui-bg-elevated)]/95 lg:block">
        <div class="flex h-full flex-col">
          <div class="border-b border-default/70 p-5">
            <NuxtLink
              to="/"
              class="flex items-center gap-3 rounded-xl border border-default/70 bg-default/30 p-3 transition hover:bg-default/50"
            >
      ...
      >
        <div v-if="sidebarOpen" class="fixed inset-0 z-40 bg-black/70 lg:hidden" @click="closeSidebar" />
      </Transition>
      ...
      >
        <div class="mb-4 flex items-center justify-between rounded-xl border border-default/70 bg-default/30 p-3">
      ...
      <div class="relative flex min-h-screen flex-col">
        ...
        <main class="flex-1 px-4 pb-4 pt-6 sm:px-6 lg:px-8">
          <div class="mx-auto w-full max-w-7xl">
            <div class="rounded-2xl border border-default/80 bg-default/20 p-4 shadow-2xl shadow-black/10 sm:p-6">
              <slot />
            </div>
          </div>
        </main>
